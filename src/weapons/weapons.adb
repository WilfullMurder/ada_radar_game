...
package body Weapons is
   overriding procedure Initialize(Self : in out Weapon_Base; Owner : not null access Entity.Entity_Interface'Class) is
   begin
      Self.Owner := Owner;
   end Initialize;
   ...
   begin
      if Self.Auto_Fire and then Self.Can_Fire(Delta_Time) and then Delta_Time >= GL.Types.Double(Self.Next_Fire_Time) then
   ...
   overriding procedure Set_Target(Self : in out Weapon_Base; Target : Entity.Entity_Ref) is
   begin
      Self.Target := Target;
   end Set_Target;
